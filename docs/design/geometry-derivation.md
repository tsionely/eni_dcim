...
| e_z_clamp_m (measurement bound) | 0.45 | Derivation row (advisory-11 §1 rider): constrained to (corridor − floor, outer plausibility] — it must EXCEED the corridor (0.30) so a saturated reading still fails admission (0.45 > 0.30 ✔), and stay below the row formula's outer plausibility at capture ranges (~0.8·d*). Kept honest so admission and the continuous test can SEE and refuse an off-corridor arrival; clamping measurement to 0.10 would blind the epistemology exactly where it must block (safety fixture pins the resurrection) | MEASUREMENT-DOMAIN, DERIVED |
| — 0.45 collision note | — | `e_z_clamp_m` (oracle measurement bound, epistemology organ) and `abort_offset_m` (commit lateral-corridor breach threshold, planner organ) are DISTINCT constants whose values coincide at 0.45 by history, not by shared derivation. Advisory-11 §1 rider (ii): recorded here so the next d*-class confusion is not born of it; `abort_offset_m` re-derives at R5 (quarantined), after which the coincidence dissolves | DOCUMENTED |
