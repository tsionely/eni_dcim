...
| R11 | הסים מתעלם לחלוטין מפקודות מהירות (SET_POSITION_TARGET) — הוכח ב-Phase 1f (מצב B) | סולם הבקרה המקורי (velocity תחילה) בטל | ‏`att_rate` הוא ה-backend הראשי כעת: קסקדת PID עם פיצוי הטיה בדחף, מהירות-מראייה כמשוב (נגזרת pose של השער), ‏hover_thrust מכויל ב-probe H | הקסקדה עוברת שער מלא ב-mock; אימות אמיתי ב-Phase 2a |
