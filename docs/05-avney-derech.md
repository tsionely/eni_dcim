...
| R7 | וידאו שותק על 5600 וטלמטריית 14550 "קפואה" בכל המדידות המוקדמות | חוסם ראייה ושערוך מצב כאחד | ‏Phase 1c הפריך את השערת ה-connect: ‏14560/5601 הם ככל הנראה סוקטי המקור היוצאים של הסים; המדידות ה"קפואות" נלקחו עם תהליך סים יתום ברקע או בסים בתפריט — ייתכן שהכול תקין במרוץ פעיל נקי. ‏Phase 1d מודד חיות-ערכים ופריימים תוך-כדי מעבר לתפריט→מרוץ (‏phase1_check עם חלונות 10s). תמיכת connect/subscribe נשארת בקוד ליתר ביטחון | באימות Phase 1d |
...
| R9 | הסים דורש "THROTTLE DOWN" לשחרור מנועים — הרחפן נשאר על הכן בכל המרוצים | חוסם כל תנועה | מצב `THROTTLE_DOWN` ב-FSM (החזקת thrust=0 אחרי חימוש, `control.throttle_down_s`); ‏`scripts/control_probe.py` מזהה איזה ממשק שליטה מזיז בפועל | ‏handshake ממומש; אימות ב-Phase 1f |
| R10 | ציר zgyro משודר קפוא (v1.0.3385); ‏baro/mag = NaN | ‏yaw מג'ירו לא זמין → פיצוי NED והחיפוש נשענים על ציר מת | בדיקות חיות עברו לרב-צירי; אם zgyro קפוא גם בטיסה — ‏yaw מראייה (מעקב שער) במקום אינטגרציית ג'ירו | ממתין לטיסה אמיתית (Phase 2a) |
| R11 | הסים מתעלם לחלוטין מפקודות מהירות (SET_POSITION_TARGET) — הוכח ב-Phase 1f (מצב B) | סולם הבקרה המקורי (velocity תחילה) בטל | ‏`att_rate` הוא ה-backend הראשי כעת: קסקדת PID עם פיצוי הטיה בדחף, מהירות-מראייה כמשוב (נגזרת pose של השער), ‏hover_thrust מכויל ב-probe H | הקסקדה עוברת שער מלא ב-mock; הרחפן זז במציאות (Phase 2a) |
| R12 | פסילת זינוק מוקדם — הסים פוסל תנועה לפני ה-GO ‏("Early Start DSQ", ‏Phase 2a) | פסילה בכל מרוץ | ה-FSM ממתין ב-THROTTLE_DOWN עד שינוי בפועל ב-`race_start_boot_time_ms` (הדגל עצמו דביק); ‏timeout ‏(`control.go_timeout_s`) למניעת נעילה בבדיקות חופשיות | ממומש; אימות Phase 2b |
| R13 | מוסכמת סימן קצבי-מנח בסים שונה מהתקן (pitch נמדד הפוך, ‏Phase 2a) | משוב חיובי → התהפכות מיידית | פרמטרי סימן פר-ציר (`rate_sign_roll/pitch/yaw`); ‏probe D פועם כעת בכל שלושת הצירים להצמדת המוסכמה; ‏`--patch` מאפשר ניסוי בלי עריכת config | באפיון Phase 2b |
