...
| R12 | פסילת זינוק מוקדם — ‏`race_start_boot_time_ms` מתעדכן בתחילת הספירה עם חותמת **עתידית**, והמראה על עצם השינוי = פסילה (2688ms, ‏Phase 2b) | פסילה בכל מרוץ | ‏GO דו-תנאי: שינוי ב-race_start **וגם** ‏`sim_boot_time_ms >= race_start_boot_time_ms`; ‏timeout לבדיקות חופשיות | ממומש; אימות Phase 2c |
| R13 | מוסכמת סימן קצבי-מנח הפוכה מהתקן **בכל שלושת הצירים** (נמדד ב-probe D, ‏Phase 2b) | משוב חיובי → התהפכות מיידית | ‏`rate_sign_roll/pitch/yaw = -1` כברירת מחדל בקונפיג; ה-mock משקף את אותה מוסכמה כך שאותו קונפיג טס בשניהם | מכויל ומוגדר |
