...
def main():
    ...
    # Prefer per-flight closest from collect_control_dz (same filters)
    closest_dz = []
    all_dz = []
    for row in per:
        if row.get("error"):
        ...
        # rebuild pooled from samples_head is incomplete — re-scan
    ...
    # Also report mean absolute closest as bias context
