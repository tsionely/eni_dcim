...
RUN_ID = "2026-07-15-rebaseline-v2-1998e5c"
...
@contextmanager
def mock_session(label: str, port_offset: int):
...
def run_campaign_attempt(attempt: int, base_params: ParamSet) -> tuple[bool, dict[str, float], list[dict], list[dict]]:
    ...
    db.record_campaign(
        f"rebaseline-v2-cem-attempt-{attempt}",
        "CEM",
        ...
        for idx in range(1, 41):
            ...
            try:
                ...
                db.record_flight(
                    ...
                    result,
                    score,
                    campaign_id=f"rebaseline-v2-cem-attempt-{attempt}",
                )
...
def write_summary(best_params: dict[str, float], campaign_rows: list[dict],
                  ...
                  cpu_rows: list[dict], attempt: int, valid_campaign: bool) -> None:
    ...
    lines = [
        f"# Re-Baseline v2 Mock Campaign - {RUN_ID}",
        "",
        "Role: QA & MOCK-TUNER.",
        "",
...
def main() -> int:
    ...
    print(f"[rebaseline-v2] commit={commit_hash()}", flush=True)
    ...
    write_summary(best_params, campaign_rows, default_rows, best_rows,
                  cpu_rows, attempt, valid_campaign)
    print(f"[rebaseline-v2] summary={OUT_DIR / 'summary.md'}", flush=True)
